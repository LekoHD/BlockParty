...
public class WinnerCountdown {
					...
					if (WinnerCountdown.number != 1.0D) {
						...
						try {
							for (String playerName : Players.getPlayersOnFloor(arenaName)) {
								System.out.print(BlockParty.messageManager.CONSOLE_OUTPUT_WINNER.replace("$PLAYER", playerName).replace("$ARENANAME$", arenaName));
	...
	public static void shootFireworks(String arenaName) {
        ...
        Firework fw = (Firework) BlockParty.getArena.get(arenaName).getWorld().spawnEntity(BlockParty.getArena.get(arenaName).getGameSpawn(), EntityType.FIREWORK);
